 ...
 */
class Version20171206160924 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__post AS SELECT id, user_id, message FROM post');
        $this->addSql('DROP TABLE post');
        $this->addSql('CREATE TABLE post (id INTEGER NOT NULL, user_id INTEGER DEFAULT NULL, message VARCHAR(140) NOT NULL COLLATE BINARY, PRIMARY KEY(id), CONSTRAINT FK_5A8A6C8DA76ED395 FOREIGN KEY (user_id) REFERENCES fos_user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO post (id, user_id, message) SELECT id, user_id, message FROM __temp__post');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__post AS SELECT id, user_id, message FROM post');
        $this->addSql('DROP TABLE post');
        $this->addSql('CREATE TABLE post (id INTEGER NOT NULL, user_id INTEGER DEFAULT NULL, message VARCHAR(140) NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO post (id, user_id, message) SELECT id, user_id, message FROM __temp__post');
        $this->addSql('DROP TABLE __temp__post');
        $this->addSql('CREATE INDEX IDX_5A8A6C8DA76ED395 ON post (user_id)');
        $this->addSql('DROP INDEX UNIQ_C74F21955F37A13B');
